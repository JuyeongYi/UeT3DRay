...
def test_extract_class_pattern() -> None:
    r = AssetResolver()
    assert r.extract_target_path("Class'/Game/Lib.Lib:RigVMModel.F'") == "/Game/Lib.Lib:RigVMModel.F"

def test_extract_redirect_chain_takes_last_quoted() -> None:
    r = AssetResolver()
    raw = "Redirect'/Old.Old:RigVMModel.G'->'/Game/Lib.Lib:RigVMModel.F'"
    assert r.extract_target_path(raw) == "/Game/Lib.Lib:RigVMModel.F"
...
def test_extract_raw_path_no_quotes() -> None:
    r = AssetResolver()
    assert r.extract_target_path("/Game/Lib.Lib:RigVMModel.F") == "/Game/Lib.Lib:RigVMModel.F"

def test_extract_invalid_returns_none() -> None:
    assert AssetResolver().extract_target_path("not a ref") is None

def test_extract_empty_string() -> None:
    assert AssetResolver().extract_target_path("") is None
